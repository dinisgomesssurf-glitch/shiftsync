...
// Each member gets a unique, stable color from the palette.
function buildColorMap(members){
  const map = {}
  ;(members || []).forEach((m, i) => { map[m.name] = SCOLORS[i % SCOLORS.length] })
  return map
}
                        ...
                        return(
                          <div key={sh.id||sh.name+di+si}
                            ...
                            style={{
                              ...
                              boxShadow: sameStart > 1 ? '0 1px 3px rgba(0,0,0,0.15)' : 'none'
                            }}>
        ...
        <div className="legend">
          {members.map(m=>(
            <div key={m.id||m.name} className="legend-item">
              <div className={`legend-dot ${colorFor(colorMap, m.name)}`}></div>{m.name}
            </div>
          ))}
        </div>
      )}
